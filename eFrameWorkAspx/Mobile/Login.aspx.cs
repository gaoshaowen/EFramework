    ...
    public partial class Login : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request.Form["yhm"] != null)
            {
                ...
                string sql = "Select top 1 UserID,YHM,MM,SiteID From a_eke_sysUsers Where delTag=0 and Active=1 and UserType>1 and YHM='" + Request.Form["yhm"].ToString() + "'";
                ...
                else
                {
                    if (eBase.GetMD5(Request.Form["mm"].ToString(), 16) == tb.Rows[0]["mm"].ToString() || Request.Form["mm"].ToString() == tb.Rows[0]["mm"].ToString() || Request.Url.Host.ToLower().IndexOf("demo.eketeam.com") > -1)
                    {
                        ...
                        user.Save();
                        eOleDB.Execute("update a_eke_sysUsers set LoginCount=LoginCount+1,LastLoginTime=getdate() where UserID='" + tb.Rows[0]["UserID"].ToString() + "'");
                        //用户登录日志
