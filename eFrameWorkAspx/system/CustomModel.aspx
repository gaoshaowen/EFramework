<%@ Page Title="" Language="C#" MasterPageFile="~/Master/systemMain.Master" AutoEventWireup="true" CodeBehind="CustomModel.aspx.cs" Inherits="eFrameWork.system.CustomModel" %>
...
<div id="SearchItems"><%=model.getSearchFilter()%></div>
...
<tr>
<td colspan="4" class="title" style="text-align:left;padding-left:125px;">
<a class="button" href="javascript:;" onclick="if(frmsearch.onsubmit()!=false){frmsearch.submit();}"><span><i class="search">搜索</i></span></a>

<a class="button" href="javascript:;" style="margin-left:15px;" onclick="saveSearch(frmsearch);"><span><i class="save">保存</i></span></a>
</td>
</tr>
</table>
</form>
...
<ev:eListColumn ControlType="text" Field="FullName" Width="120" FieldName="姓名" Move="true" Size="true" OrderBy="true" runat="server" />
<ev:eListColumn ControlType="password" Field="PassWord" FieldName="密码" Width="90" Move="true" Size="true" OrderBy="true" runat="server" />
<ev:eListColumn ControlType="date" Field="Birthday" FieldName="生日" Width="100" FormatString="{0:yyyy-MM-dd}" Move="true" Size="true" OrderBy="true" runat="server" />
<ev:eListColumn ControlType="text" Field="Sex" FieldName="性别" Width="80" Options="[{text:男,value:1},{text:女,value:2}]" ReplaceString="[{text:不详,value:0}]" Move="true" Size="true" OrderBy="true" runat="server" />
<ev:eListColumn ControlType="text" Field="Hobby" FieldName="兴趣爱好" BindAuto="True" BindObject="Demo_Dictionaries" BindText="Name" BindValue="ID" BindOrderBy="PX" BindCondition="delTag=0 and show=1 and PID=5" Move="true" Size="true" OrderBy="true" runat="server" />
<ev:eListColumn ControlType="text" Field="Education" FieldName="学历" Width="90" BindAuto="True" BindObject="Demo_Dictionaries" BindText="Name" BindValue="ID" BindOrderBy="PX" BindCondition="delTag=0 and show=1 and PID=1" Move="true" Size="true" OrderBy="true" runat="server" />
<ev:eListColumn ControlType="file" Field="Photo" FieldName="照片" Move="true" Size="true" OrderBy="true" runat="server"><div style="height:{row:height}px;"><span></span><p><img style="max-width:180px;" src="{data:photo}" oner3ror="this.style.display='none';"></p></div></ev:eListColumn>
<ev:eListColumn ControlType="text" Field="Height" FieldName="身高" Width="90" FormatString="{0:F2}" Move="true" Size="true" OrderBy="true" runat="server" />
<ev:eListColumn ControlType="text" Field="Show" FieldName="是否显示" Width="90" Options="[{text:是,value:True},{text:否,value:False}]" Move="true" Size="true" OrderBy="true" runat="server">
<a href="{base:url}act=show&id={data:id}&value={data:showvalue}"><img src="{base:VirtualPath}{data:ShowPIC}" border="0"></a>
</ev:eListColumn>
<ev:eListColumn ControlType="sort" Field="PX" FieldName="显示顺序" Width="90" Move="true" Size="true" OrderBy="true" runat="server" />
<ev:eListColumn ControlType="text" Field="addTime" FieldName="添加时间" Width="140" FormatString="{0:yyyy-MM-dd HH:mm:ss}"  Move="true" Size="true" OrderBy="true" runat="server" />
<ev:eListColumn ControlType="text" FieldName="操作" Width="130" Move="true" Size="true" runat="server">
