<%@ Page Language="C#" MasterPageFile="~/Master/huiMain.Master" AutoEventWireup="true" CodeBehind="Dictionaries.aspx.cs" Inherits="eFrameWork.systemhui.Dictionaries" %>
...
function bindTree()
{
    ...
    {
        var _url = "Dictionaries.aspx?modelid=" + ModelID + "&act=setsort&id=" + li.attr("dataid") + "&pid=" + nul.attr("PID") + "&index=" + (li.index() + 1) + "&t=" + now();
        $.ajax({
            type: "GET",
            async: false,
            url: _url,
...
{
%>
<a id="A1" class="btn btn-primary radius r" style="margin-top:4px;<%=( (Action.Value == "" || Action.Value == "view" ) && model.Power["Add"].ToString().ToLower()=="true" ? "" : "display:none;" )%>" class="button" href="javascript:;" dataid="<%=pid%>" onclick="add(this);"><i class="Hui-iconfont">&#xe600;</i> 添加</a>
<%
}
}
%>
...
<td class="content" id="padd"><span class="eform"><ev:eFormControl ID="M1_F2" Name="M1_F2" ControlType="autoselect" Field="ParentID" FieldName="上级分类" FieldType="uniqueidentifier" BindAuto="True" BindObject="Dictionaries" BindText="MC" BindValue="DictionarieID" BindCondition="delTag=0 and (ParentID is Null)" BindOrderBy="PX,AddTime" NotNull="False" DataType="string" runat="server" /></span></td>
</tr>
