...
*******************************************************************************/
var earrs=document.location.href.split("/");
//earrs[0]=http:
//earrs[2]=domain
var ect=earrs.length-4;

if(document.location.port.length==0) //80端口
{
	if(earrs[2].toLowerCase()=="localhost"){ect--;}
	if(earrs[2]=="127.0.0.1"){ect--;}
	if(earrs[2].indexOf("192.168.")>-1){ect--;}	
}
//var image_postURL="../../image.ashx";
//var flash_postURL="../flash.ashx";
//var media_postURL="../media.ashx";
//var accessory_postURL="../uploadaccessory.ashx";

var image_postURL="../../../ProUpload.aspx?type=image";
var flash_postURL="../../ProUpload.aspx?type=flash";
var media_postURL="../../ProUpload.aspx?type=media";
var accessory_postURL="../../ProUpload.aspx?type=accessory";


var kePath="";
for(i=0;i<ect;i++)
{
	//if(i==0){image_postURL+="?path=";}
	//image_postURL+="../";
	kePath+="../";
}
//alert(ect + "___"+earrs[4]  + ":::" + earrs.length);
...
//未上控件：'inputtext','vartext','ekecontrol'
KE.setting = {	
	wyswygMode : true,
	loadStyleMode : true,
	resizeMode : 2,
	...
	items : [
		...
		'newflash', 'newmedia','accessory', 'advtable', 'hr', 'link', 'unlink', '|', 'about'
	],
	colorTable : [
	...
	htmlTags : {
		...
		a : ['href', 'target', 'name'],
		embed : ['src', 'width', 'height', 'type', 'loop', 'autostart','wmode', 'quality', '.width', '.height', 'align', 'allowscriptaccess', '/'],
...
KE.tempImage = null;
KE.setTempImage=function(obj){KE.tempImage=obj;};
KE.g = {};
KE.plugin = {};
KE.$ = function(id, doc){
...
KE.selection = function(doc) {
	...
	this.init = function() {
		...
		//parent.document.title=sel.createRange().text + "::" + sel.type;
		//parent.document.title=sel.createControlRange().text
...
obj:null,
	...
	getMediaType : function(src) {
		...
		else if (src.match(/\.(swf|flv|f4v|mp4)(\?|$)/i)) return 'flash';
		else return 'media';
	},
...
//alert(attrs.src);
		...
		html += 'kesrctag="' + escape(srcTag) + '" onclick="parent.KE.setTempImage(this);" />';
...
KE.dialog = function(arg){
	...
	this.zIndex = 19999016;
	if(typeof(getMaxZindex)=="function")
	{
		//alert(3);
		//正在
		//alert("fun" + getMaxZindex(KE.util.getDocumentElement()));
	}
...
KE.create = function(id, mode) {
	...
	//iframe.style.backgroundColor="#ff0000";
	iframe.setAttribute("frameBorder", "0");
	...
	//KE.event.add(iframeWin, 'focus', function(e) 
	//{
		//扩展事件
		//iframeDoc.body.style.backgroundColor = '#ddd';
	//});
	//KE.event.add(iframeWin, 'blur', function(e) 
	//{
		//扩展事件
...
/*
			if (e.keyCode == 46) {
				var html=KE.selectedHtml(id);
				var reg = /(src|href|background|value|flashvars)=(['""])?([^'"">\s]*?)(\2|\s|>)/ig;
				var tmp="";
//自动删除图片
				while(reg.exec(html) != null)
				{
					tmp=RegExp.$3;
					if (tmp.indexOf("images/temp/") > -1)
					{
						tmp = tmp.replace("../", "");
						if (tmp.indexOf("vcastr_file=") > -1) tmp = tmp.replace("vcastr_file=", "");
						if (tmp.indexOf("f=") > -1) tmp = tmp.replace("f=", "");
						if (tmp.indexOf("&") > -1)
						tmp = tmp.substring(0, tmp.indexOf("&"));
						var ajax=null;
						if (window.ActiveXObject) {ajax=new ActiveXObject("Microsoft.XMLHTTP");}
						else{ajax=new XMLHttpRequest();}
						var url="upload.aspx?act=del&obj=null&file=" + tmp;
...
//if(startNode.className === "ke-flash")
//{
	//document.title=startNode.offsetWidth;
//startNode.style.width="550px";
//startNode.style.height="550px";
//}
	...
	var htmlvalue=srcTextarea.value;
	htmlvalue=htmlvalue.replace(/〈/g,"&lt;").replace(/〉/g,"&gt;");
	KE.util.setFullHtml(id, htmlvalue);
	KE.history.add(id, 0);
...
KE.init = function(args) {
	...
	//alert(args.height);
//alert(args.imageUploadJson);
	if(args.imageUploadJson)
	{
		KE.path=KE.request(args.imageUploadJson,"imagepath");
	}
	else
	{
		g.imageUploadJson=image_postURL;
		
	}
	if(args.maxwidth)
	{
		g.imageUploadJson += "&maxwidth=" + args.maxwidth;
	}

	if(args.flashUploadJson)
	{
		KE.path=KE.request(args.flashUploadJson,"flashpath");
	}
	else
	{
		g.flashUploadJson=flash_postURL;		
	}
	if(args.mediaUploadJson)
	{
		KE.path=KE.request(args.mediaUploadJson,"mediapath");
	}
	else
	{
		g.mediaUploadJson=media_postURL;		
	}
	if(args.accessoryUploadJson)
	{
		KE.path=KE.request(args.accessoryUploadJson,"accessorypath");
	}
	else
	{
		g.accessoryUploadJson=accessory_postURL;		
	}



	//alert(g.imageUploadJson);
	KE.path=kePath;
	g.config = {};
	g.undoStack = [];
	g.redoStack = [];
...
KE.show = function(args) {
	...
	if(typeof linkArrys === "undefined")
	{

		//alert(KE.setting.items.length);
			/*
			KE.setting.items=[
		'source', '|', 'fullscreen', 'undo', 'redo',
		'|', 'justifyleft', 'justifycenter', 'justifyright',
		...
		'title', 'fontname', 'fontsize', '|', 'textcolor', 'bgcolor', 'bold',
		'italic', 'underline', 'strikethrough', 'removeformat', '|', 'image',
		'newflash', 'newmedia','accessory', 'advtable', 'hr', 'link', 'unlink', '|', 'about','ekecontrol'
	];
			*/
	}
	KE.init(args);
...
KE.lang = {	
	...
	newalign:'对齐方式',
	newmedia : '插入多媒体',
	invalidnewmedia : "请输入有效的URL地址。\n只允许mp3,swf,flv,f4v,mp4,wav,wma,wmv,mid,avi,mpg,asf,rm,rmvb格式。",
	newflash : '插入FLASH',
	invalidnewflash : "请输入有效的URL地址。\n只允许swf,flv,f4v,mp4格式。",
	accessory : '插入附件',
	invalidAccessory : "请输入有效的URL地址。\n只允许doc,docx,rtf,csv,xls,xlsx,ppt,rar,zip,pdf,txt,gif,jpg,jpeg,png,bmp格式。",
	source : 'HTML代码',
	undo : '后退(Ctrl+Z)',
	redo : '前进(Ctrl+Y)',
	cut : '剪切(Ctrl+X)',
...
(function (KE, undefined) 
{	 
KE.plugin['ekecontrol_bak'] = {
	click : function(id) {
		KE.util.selection(id);
		KE.util.insertHtml(id, '<a href="#" data="33">测试</a>');
		KE.util.focus(id);
	}
};
KE.plugin['ekecontrol'] = {
click : function(id) {
if(typeof linkArrys === "undefined")
{
	return;
...
for (var i = 0, len = linkArrys.length; i < len; i++) 
{
	var value = linkArrys[i];
	var txt=value.split(",")[2];
	var html = '<span class="ke-reset">' + txt + '</span>';
	menu.add(
				html,
				(function(value) {
					return function() {
						KE.plugin['ekecontrol'].exec(id, value);
					};
				})(value),{}
			);
}
...
exec : function(id, value) {
		...
		//KE.util.insertHtml(id, '<a href="#" data="'+data+'">'+txt+'</a> ');
		KE.util.insertHtml(id, '{' + type + ':'+data+'}');
		KE.util.focus(id);
		if(typeof linkArrys === "undefined") return;
		var idx=-1;
		for (var i = 0, len = linkArrys.length; i < len; i++) 
		{
			if(value==linkArrys[i])
			{
				idx=i;
				break;
			}
		}
		linkArrys.splice(idx,1);
	}
};
...
KE.plugin['inputtext'] = {
	click : function(id) {
		KE.util.selection(id);
		KE.util.insertHtml(id, '<input type="text" >');
		KE.util.focus(id);
	}
};
KE.plugin['vartext'] = {
click : function(id) {
if(typeof TextArray === "undefined") return;
var menu = new KE.menu({
			id : id,
			cmd : 'vartext',
			width : 260
...
for (var i = 0, len = TextArray.length; i < len; i++) 
{
	...
	menu.add(
				...
				(function(value) {
					return function() {
						KE.plugin['vartext'].exec(id, value);
					};
				})(value),{}
			);
}
...
KE.plugin['newalign'] = {
	click : function(id) {
		var aligns = ['左对齐', '居中对齐', '右对齐', '两端对齐'];
		var cmd = 'newalign';
		KE.util.selection(id);
		var size = KE.queryCommandValue(KE.g[id].iframeDoc, 'newalign');
		var menu = new KE.menu({
			id : id,
			cmd : cmd,
			width : 120
		});
		for (var i = 0, len = aligns.length; i < len; i++) {
			var value = aligns[i];
	...
	exec : function(id, value) {
		...
		//cmd.wrap('span', [{'.vertical-align': value}]);
		//KE.util.execOnchangeHandler(id);
		var tmp='justifyfull';
		 if(value.indexOf('左')>-1){tmp='justifyleft';}
		 if(value.indexOf('右')>-1){tmp='justifyright';}
		if(value.indexOf('中')>-1){tmp='justifycenter';}
		KE.util.execCommand(id, tmp, null);
		this.menu.hide();
		KE.util.focus(id);
	}
};
...
KE.plugin['image'] = {
	...
	check : function(id) {
		...
		if(urlBox.value.length==0 || urlBox.value=="http://")
		{
			alert(type == 2 ? "请选择本地文件!" : "请输入图片完整地址!");
			return false;
		}
		if (!urlBox.value.match(/\.(jpg|jpeg|gif|bmp|png|jpeg|tif)(\s|\?|$)/i)) {
	...
	exec : function(id) {
		...
		if (type == 2) {
			...
			var onloadFunc = function() {
				...
				var data = '';
				try {
					//data = KE.util.parseJson(uploadDoc.body.innerHTML.replace("<\/pre>","").replace("<pre>",""));
					data = KE.util.parseJson(uploadDoc.body.innerHTML.replace(/<[^>]*>|<\/[^>]*>/gm,""));
//alert(uploadDoc.body.innerHTML.replace(/<[^>]*>|<\/[^>]*>/gm,""));
				} catch(e) {
					alert(KE.lang.invalidJson);
				}
				if (typeof data === 'object' && 'errcode' in data) {
					if (parseInt(data.errcode) === 0) {						
						//self.insert(id, data.url, title, width, height, 0, align);
						for(var i=0;i<data.files.length;i++)
						{
							self.insert(id, data.files[i].url, title, width, height, 0, align);
						}
					} else {
						alert(data.message);
						return false;
	...
	insert_bak : function(id, url, title, width, height, border, align) {
		var pos=url.lastIndexOf(".");
		var ext = url.substr(pos, url.length - pos);
		var url1=url.replace(ext,"_1.jpg");
	
		var html = '<table border="0" cellpadding="0" cellspacing="0" width="1"><tr><td background="'+ url1 +'">';
		html += '<img src="' + url + '" kesrc="' + url + '" ';
		if (width > 0) html += 'width="' + width + '" ';
		if (height > 0) html += 'height="' + height + '" ';
		if (title) html += 'title="' + title + '" ';
		if (align) html += 'align="' + align + '" ';
		html += 'alt="' + title + '" ';
		html += 'border="' + border + '" /></td></tr></table>';
...
KE.plugin['newmedia'] = {
    ...
    check : function(id) {
		...
		var type=KE.$('type1', dialogDoc);
        var url = '';
        if (type.checked) {
            url = KE.$('flaFile', dialogDoc).value;
        } else {
            url = KE.$('url', dialogDoc).value;
        }
	if(url.length==0 || url=="http://")
		{
			alert(type.checked ? "请选择本地文件!" : "请输入完整地址!");
			return false;
		}
        
		if (!url.match(/\.(mp3|swf|flv|f4v|mp4|wav|wma|wmv|mid|avi|mpg|asf|rm|rmvb)(\s|\?|$)/i)) {		
            //alert(KE.lang['invalidnewmedia']);
            //window.focus();
            //KE.g[id].yesButton.focus();
            //return false;
		alert("文件类型不支持,只支持以下格式\n(mp3|swf|flv|f4v|mp4|wav|wma|wmv|mid|avi|mpg|asf|rm|rmvb)");
		return false;
    ...
    exec : function(id) {
        ...
        if (type.checked) {
            ...
            var url = KE.$('url', dialogDoc).value;
            var w = KE.$('flaWidth', dialogDoc).value;
            var h = KE.$('flaHeight', dialogDoc).value;
            this.insert(id, url, w, h);
        }
    },
    insert : function(id, url, w, h) {
		
	var html='';
	if(w.length==0)w="450";
	if(h.length==0)h="250";
	if (url.match(/\.(flv|f4v|mp4)(\s|\?|$)/i)) 
	{	
		html = '<embed src="images/player.swf" wmode="transparent" flashvars="f='+url+'&v=85&b=#000000&x=images/player.xml" allowfullscreen="true" quality="high" pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash"';
		html +=' width="600" height="506"';
		html += '></embed>';
	}
	else
	{
		html = '<embed src="' + url + '" wmode="transparent"  type="video/x-ms-asf-plugin"';      
...
KE.plugin['newflash'] = {
	...
	if(url.length==0 || url=="http://")
		...
		if (!url.match(/\.(swf|flv|f4v|mp4)(\s|\?|$)/i)) {		
            //alert(KE.lang['invalidnewflash']);
            //window.focus();
            //KE.g[id].yesButton.focus();
            //return false;
    ...
    insert : function(id, url, w, h) {
		...
		if(h.length==0)h="250";
		var html='';
		if (url.match(/\.(swf)(\s|\?|$)/i))
		{
			html = '<embed src="' + url + '" wmode="transparent"  quality="high" pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash"';
			html+=' width="'+w+'" height="'+h+'"';
			html += '></embed>';
		}
		else
		{
			html = '<embed src="images/player.swf" wmode="transparent" flashvars="f='+url+'&b=#000000&v=85&x=images/player.xml" allowfullscreen="true" quality="high" pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash"';
			html+=' width="600" height="506"';
			html += '></embed>';
...
KE.plugin['accessory'] = {
	...
	{
		case "doc":
		   img=KE.path + "images/ico/doc.gif";
			break;
		case "docx":
		   img=KE.path + "images/ico/doc.gif";
			break;
		case "rtf":
		   img=KE.path + "images/ico/rtf.gif";
			break;
		case "csv":
		   img=KE.path + "images/ico/csv.gif";
			break;
		case "xls":
		   img=KE.path + "images/ico/xls.gif";
			break;
		case "xlsx":
		   img=KE.path + "images/ico/xls.gif";
			break;
		case "ppt":
		   img=KE.path + "images/ico/ppt.gif";
			break;
		case "rar":
		   img=KE.path + "images/ico/rar.gif";
			break;
		case "zip":
		   img=KE.path + "images/ico/zip.gif";
			break;
		case "txt":
		   img=KE.path + "images/ico/txt.gif";
			break;
		case "pdf":
		   img=KE.path + "images/ico/pdf.gif";
			break;
		default:
		   img=KE.path + "images/ico/unknown.gif";
	}
...
//parent.document.title= width;
//var g = KE.g[id];
//var startNode = g.keRange.startNode;
//var endNode = g.keRange.endNode;
//if(startNode!=endNode ){ return KE.util.getMediaEmbed(attrs);}
//startNode.style.width=attrs.width  + "px";
//startNode.style.height=attrs.height  + "px";
//if(KE.tempImage)
//{
//if(startNode!=endNode ){ return KE.util.getMediaEmbed(attrs);}
//startNode.style.width=attrs.width  + "px";
//startNode.style.height=attrs.height  + "px";
//KE.tempImage.style.width=attrs.width  + "px";
//KE.tempImage.style.height=attrs.height  + "px";
//}
...
KE.plugin['media'] = {
	...
	check : function(id, url, width, height) {
		var dialogDoc = KE.util.getIframeDoc(this.dialog.iframe);
		if (!url.match(/^.{3,}\.(swf|flv|f4v|mp4|mp3|wav|wma|wmv|mid|avi|mpg|mpeg|asf|rm|rmvb)(\?|$)/i)) {
